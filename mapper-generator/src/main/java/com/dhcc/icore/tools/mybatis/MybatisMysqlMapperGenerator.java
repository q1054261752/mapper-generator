...
import static com.dhcc.icore.tools.mybatis.Logger.log;
public class MybatisMysqlMapperGenerator
{
    private final GeneratorConfig config;

    public MybatisMysqlMapperGenerator(GeneratorConfig config) {
    ...
    public void generate() {
        ...
        try{
            ...
            for (Map.Entry<String, List<String>> entry : includeTables.entrySet())
            {
                ...
                String daoInterfaceOutputDir = config.getJavaClassOutDir() + File.separator+ entry.getKey();
                ...
                connection = DriverManager.getConnection(this.config.getJdbcUrl(), this.config.getJdbcUserName(), this.config.getJdbcPassword());
                TableResolver tableResolver = findTableResolver(this.config.getJdbcDriver());
                ...
                for (Table table : resolvedTables.values()){
                    ...
                    String mapperName = mapperOutputDir + File.separator + entityName + "." + this.config.getMapperSuffix() + ".xml";
    ...
    private void generateEntity(String outName, String type, Table table)
    {
        ...
        try
        {
            ...
            entityOut.write(stringWriter.toString().getBytes(this.config.getCharset())); return;
        }
        catch (Exception e)
        {
    ...
    private void generateDaoImpl(String outName, String type, Table table)
    {
        ...
        List<String> importedClass = new ArrayList();
        importedClass.add(this.config.getBasicDaoImplFQCN());
